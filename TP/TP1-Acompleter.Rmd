...
```{r}
A = data.frame(y=as.vector(as.matrix(t(ozone[,1:11]))),x=rep(colnames(ozone[,1:11]),nrow(ozone[,1:11])))
...
```{r}
respca = PCA(ozone,scale.unit=T,quali.sup=c(12,13),graph=F)
...
relation entre la concentration maximale d'ozone (maxO3) avec la direction du vent (vent) et la précipitation (pluie). 
...
taux maximal d'ozone (*maxO3*) par rapport à celui de la veille (*maxO3v*) et en fonction des précipitations (*pluie*). Pour cela, on va mettre en place un modèle d'analyse de la covariance.

## Question 1 
A l'aide des commandes suivantes, représentez le nuage de points entre la
variable à expliquer *maxO3* et la variable quantitative explicative *maxO3v*, en fonction des modalités de la variable qualitative *pluie*. Commentez. 
