...
```{r,eval=F}
df=data.frame(tau = rep(fitlasso$lambda,ncol(tildeX)), theta=as.vector(t(fitlasso$beta)),variable=rep(colnames(tildeX),each=length(fitlasso$lambda)))
...
```{r,eval=F}
fitEN <- glmnet(...)
df=data.frame(tau = rep(fitEN$lambda,ncol(tildeX)), theta=as.vector(t(fitEN$beta)),variable=rep(c(colnames(tildeX)),each=length(fitEN$lambda)))
...
```{r,eval=F}
EN_cv <- cv.glmnet(tildeX, tildeY, alpha = 0.5, lambda = tau_seq,nfolds=10,type.measure=c("mse"),intercept=F)
g3=g3 + geom_vline(xintercept = EN_cv$lambda.min,linetype="dotted", color = "red")+ geom_vline(xintercept = EN_cv$lambda.1se,linetype="dotted", color = "green")
ggplotly(g3)
```
...
A<-data.frame(y=as.vector(as.matrix(t(SAheart[,-c(5,10)]))),x=rep(colnames(SAheart[,-c(5,10)]),
                                                                  nrow(SAheart[,-c(5,10)])))
...
```{r}
Fourmis <- read.table("Fourmis.txt",header=T, sep=',')
...
Est-ce-qu'une modélisation avec un modèle prenant la sur-dispersion est plus appropriée ? 
